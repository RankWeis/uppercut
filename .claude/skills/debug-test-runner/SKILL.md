...
```bash
./gradlew -p testProjects/karate-v2 printTestCp -I cp.gradle -DcpOut=/tmp/testcp.txt
./gradlew :KarateTestRunner:jar
```

Then run the main class with that classpath **plus** the runner jar, from the configured working
directory (`src/test`), passing the same arguments the IDE passes:

```bash
java -cp "$(cat /tmp/testcp.txt);KarateTestRunner/build/libs/KarateTestRunner.jar" \
