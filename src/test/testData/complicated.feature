Feature: Nested JSON and Dynamic Expressions
  ...
  Scenario: Chain API calls with dependencies
    ...
    And r = function() {
      ...
      for (i=0 ; i< r.length; i++){     r[i].click();     }}
    When method POST
    Then status 201
  ...
  Scenario: Perform nested operations on a JSON structure
    ...
    * match result ==
  [
      { userId: 1, details: [{ detailId: 1, info: 'Alice_info1' }, { detailId: 2, info: 'Alice_info2' }, { detailId: 3, info: 'Alice_info3' }] },
      { userId: 2, details: [{ detailId: 1, info: 'Bob_info1' }, { detailId: 2, info: 'Bob_info2' }, { detailId: 3, info: 'Bob_info3' }] }
  ]
  ...
  Scenario: Generate JSON dynamically using embedded JavaScript
    ...
    * match dynamicJson ==
  {
  key1: { value: 10, nested: { id: 'item1' } },
  key2: { value: 20, nested: { id: 'item2' } },
  key3: { value: 30, nested: { id: 'item3' } },
