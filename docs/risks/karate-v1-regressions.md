...
| R6 | Appender still installs before the version branch | `getOutputStreamAppender()` runs before `parseArgs`, so a *v2* project that supplies its own logback gets its console appenders detached and output prefixed `<<UPPERCUT>>` — v1-shaped noise on the v2 console (review finding 03). Not a v1 regression per se, but the fix (move install after arg parsing, v1-only) touches the v1 startup order and must keep the appender installed before any v1 test output is produced | Review finding open; v1 leg of the UI test would catch a mis-ordered fix | **Open** |
